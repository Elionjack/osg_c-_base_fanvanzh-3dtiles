...
static std::vector<std::vector<unsigned char>> build_atlases(
    const tinygltf::Model& model, int atlas_count, int atlas_size,
    int grid, int jpeg_quality) {
    ...
    std::vector<std::vector<unsigned char>> encoded(atlas_count);
    for (int i = 0; i < atlas_count; ++i)
        encoded[i] = encode_jpeg(pixels[i], atlas_size, atlas_size, jpeg_quality);
    return encoded;
}
...
static DracoResult encode_draco(const Group& group) {
    draco::Mesh mesh;
    ...
    // HLOD meshes can span many kilometres.  Eleven position bits produced
    // metre-scale quantization steps after atlas groups were encoded
    // independently, so formerly coincident borders could decode to visibly
    // different positions.  Twenty bits keeps the error near centimetre scale
    // for the current root HLOD while retaining Draco compression.
...
static tinygltf::Model build_output(
    ...
    const std::vector<std::vector<unsigned char>>& atlases,
    int atlas_size) {
    ...
    for (size_t i = 0; i < atlases.size(); ++i) {
        size_t offset = append_aligned(binary.data, atlases[i]);
        tinygltf::BufferView view;
        view.buffer = 0; view.byteOffset = offset; view.byteLength = atlases[i].size();
    ...
    for (size_t group_index = 0; group_index < groups.size(); ++group_index) {
        ...
        DracoResult draco = encode_draco(group);
...
bool optimize_hlod_glb_buffer(const std::string& input_glb,
    ...
    try {
        ...
        auto atlases = build_atlases(input, atlas_count, atlas_size, grid, jpeg_quality);
        auto groups = merge_geometry(input, atlas_count, atlas_size, grid);
        for (auto& group : groups) simplify_group(group, ratio, max_error);
        tinygltf::Model output = build_output(groups, atlases, atlas_size);
    ...
    try {
        ...
        if (options.atlases == 1) {
            ...
            if (!optimize_hlod_glb_buffer(
                    ...
                    options.max_error, options.jpeg_quality, &optimize_error))
        ...
        auto atlases = build_atlases(input, atlas_count, options.atlas_size, grid, options.jpeg_quality);
        ...
        tinygltf::Model output = build_output(groups, atlases, options.atlas_size);
        tinygltf::TinyGLTF writer;
        fs::create_directories(options.output.parent_path());
