...
// ============================================================
constexpr double kHlodGeometricErrorMultiplier = 1.55;

static double get_hlod_geometric_error(double base_cell_ge, int level) {
    return base_cell_ge * std::pow(
        kHlodGeometricErrorMultiplier, static_cast<double>(level + 1));
}
...
static QuadNode build_quadtree_impl(const SpatialGrid& grid,
    ...
    if (size == branch_side) {
        ...
        // Use one global source-cell baseline and a fixed per-level multiplier
        // so every peer has the same error and every parent is exactly 1.55x
        // its HLOD child level.
        node.geometricError = get_hlod_geometric_error(base_cell_ge, node.level);
...
QuadNode build_quadtree(const SpatialGrid& grid, int branching_factor) {
    ...
    // Use the largest source-cell half-span/root error as a common baseline.
    // HLOD levels then grow by a fixed 1.55 multiplier, independent of sparse
    // edge occupancy and source-tree depth.
